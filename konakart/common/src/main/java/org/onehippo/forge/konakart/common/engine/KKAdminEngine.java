...
public class KKAdminEngine {
   ...
   */
  public static void init(@Nonnull Session session) {
   ...
   * @param session the JCR Session
   */
  private void internalInit(@Nonnull Session session) {
  ...
  private void init() {
    ...
    try {
      ...
      throw new RuntimeException("Failed to initialize the Konakart Admin Engine", e);
    }
  }
